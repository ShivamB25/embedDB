...
class EmbedDB:
    ...
    def __init__(
        self, dimension: Optional[int] = None, model_path: Optional[str] = None
    ):
        """
    ...
    def add(self, id: str, vector: List[float], metadata: Optional[Any] = None) -> None:
        ...
        if self._dimension is None:
            ...
            raise ValueError(
                f"Vector dimension ({len(vector)}) does not match database dimension ({self._dimension})"
            )
    ...
    def search(self, query_vector: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        if len(query_vector) != self._dimension:
            raise ValueError(
                f"Query vector dimension ({len(query_vector)}) does not match database dimension ({self._dimension})"
            )
        ...
        for id, vector in self._vectors.items():
            ...
            results.append(
                {"id": id, "similarity": similarity, "metadata": self._metadata[id]}
            )
    ...
    def embed_text(self, text: str) -> List[float]:
        ...
        if self._model is None:
            ...
            if self._dimension is None:
                ...
                self._dimension = len(
                    self._model.encode("test", convert_to_numpy=False)
                )
